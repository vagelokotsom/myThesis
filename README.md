...
### **🔐 Authentication Flow (Σύνοψη)**
Μετά το login, ο backend εκδίδει JWT token που περιέχει τον ρόλο του χρήστη (ADMIN/TEACHER/STUDENT).
Το frontend αποθηκεύει το token και το στέλνει στο `Authorization` header σε κάθε request.
Το Spring Security επαληθεύει το token και εφαρμόζει role checks πριν από κάθε API endpoint.
